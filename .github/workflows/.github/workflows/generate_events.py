...
def generate_events(canton):
    ...
    prompt = f"""Génère 3 événements typiques qui pourraient avoir lieu dans le canton de {canton} en Suisse romande dans les 30 prochains jours.
...
Retourne UNIQUEMENT un tableau JSON valide, sans texte avant ou après."""
    ...
    try:
        ...
        print(f"Error parsing response for {canton}: {data}")
        return []
...
def save_to_firestore(events, canton):
    for event in events:
        ...
        print(f"Saved event: {event.get('title')} for {canton}")

for canton in CANTONS:
    print(f"Generating events for {canton}...")
